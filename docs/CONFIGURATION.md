...
|---|---|---|
| `RATE_LIMIT_PER_HOUR` | `60` | Hard cap on Claude calls per rolling hour across *all* users of the running app. Over the limit the app shows "demo is busy" and makes no call. On a single-container deploy this is the real spend bound. |
